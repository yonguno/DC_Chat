...
sio.sockets.on('connection',function(socket){
	...
	if(connections[sessionID]){
	  connections[sessionID].emit('news',{msg:'You opened new session. You are no longer use this session.'});
		socket.emit('news',{msg:'You have duplicated connections. Your old session will be disconnected.'});
		...
		if(re.test(data.msg.replace(/\s+$/,''))) {
          ...
          users[tell[2]].emit('news',{msg:" tell from "+username+": "+tell[3]});
			    socket.emit('news',{msg:" tell sent to "+tell[2]+": "+tell[3]});
				  ...
				  socket.emit('news',{msg:"Invaild username."});
				}
			}
		}else{
